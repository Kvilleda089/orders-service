...
pm.test('Status is updated to COMPLETED', function () {
...
pm.test('Response ID matches request ID', function () {
  const requestId = pm.request.url.path.slice(-2)[0];
  pm.expect(jsonData.id).to.equal(requestId);
});
